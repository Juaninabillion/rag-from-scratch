...
In recent years, monitoring has undergone a Cambrian Explosion: Riemann, Heka, Bosun, and Prometheus have emerged as open source tools that are very similar to Borgmon’s time-series–based alerting. In particular, Prometheus[42](#id-WakuESGIBIysp) shares many similarities with Borgmon, especially when you compare the two rule languages. The principles of variable collection and rule evaluation remain the same across all these tools and provide an environment with which you can experiment, and hopefully launch into production, the ideas inspired by this chapter.
...
To facilitate mass collection, the metrics format had to be standardized. An older method of exporting the internal state (known as *varz*)[43](#id-D04IaFPcKsX) was formalized to allow the collection of all metrics from a single target in one HTTP fetch. For example, to view a page of metrics manually, you could use the following command:
...
A Borgmon can collect from other Borgmon,[44](#id-kD2ukSmuvsW) so we can build hierarchies that follow the topology of the service, aggregating and summarizing information and discarding some strategically at each level. Typically, a team runs a single Borgmon per cluster, and a pair at the global level. Some very large services shard below the cluster level into many *scraper* Borgmon, which in turn feed to the cluster-level Borgmon.
...
In hindsight, it’s apparent that this schemaless textual interface makes the barrier to adding new instrumentation very low, which is a positive for both the software engineering and SRE teams. However, this has a trade-off against ongoing maintenance; the decoupling of the variable definition from its use in Borgmon rules requires careful change management. In practice, this trade-off has been satisfactory because tools to validate and generate rules have been written as well.[45](#id-ep2uzS2h7HQ)

##### Exporting Variables

Google’s web roots run deep: each of the major languages used at Google has an implementation of the exported variable interface that automagically registers with the HTTP server built into every Google binary by default.[46](#id-D04I1SWFGTjHG) The instances of the variable to be exported allow the server author to perform obvious operations like adding an amount to the current value, setting a key to a specific value, and so forth. The Go `expvar` library[47](#id-4DBukIYFGTqHo) and its JSON output form have a variant of this API.

# Collection of Exported Data

To find its targets, a Borgmon instance is configured with a list of targets using one of many name resolution methods.[48](#id-WakuoFLFEfk) The target list is often dynamic, so using service discovery reduces the cost of maintaining it and allows the monitoring to scale.
...
It’s interesting that varz is quite dissimilar to SNMP (Simple Network Management Protocol), which "is designed […] to have minimal transport requirements and to continue working when most other network applications fail" [[Mic03]](/sre-book/bibliography#Mic03). Scraping targets over HTTP seems to be at odds with this design principle; however, experience shows that this is rarely an issue.[49](#id-JJ7umIvcZfL) The system itself is already designed to be robust against network and machine failures, and Borgmon allows engineers to write smarter alerting rules by using the collection failure itself as a signal.
...
This returns the last 10 minutes of history of the time-series that matched the expression. If we were collecting data points once per minute, we would expect to return 10 data points in a 10-minute window, like so:[51](#id-gA2unS4IBieSg)
...
With the example time-series data from the query before, the results for the `task:http_requests:rate10m` rule would look like:[54](#id-Pj0uxF0SlFb)
...
The output of these rules might look like:[55](#id-YAJuDSrs9FV)
...
[42](#id-WakuESGIBIysp-marker)Prometheus is an open source monitoring and time-series database system available at [*https://prometheus.io*](https://prometheus.io).

[43](#id-D04IaFPcKsX-marker)Google was born in the USA, so we pronounce this *"var-zee."*

[44](#id-kD2ukSmuvsW-marker)The plural of Borgmon is Borgmon, like sheep.

[45](#id-ep2uzS2h7HQ-marker)Many non-SRE teams use a generator to stamp out the initial boilerplate and ongoing updates, and find the generator much easier to use (though less powerful) than directly editing the rules.

[46](#id-D04I1SWFGTjHG-marker)Many other applications use their service protocol to export their internal state, as well. OpenLDAP exports it through the `cn=Monitor` subtree; MySQL can report state with a `SHOW VARIABLES` query; Apache has its `mod_status` handler.

[47](#id-4DBukIYFGTqHo-marker)[*https://golang.org/pkg/expvar/*](https://golang.org/pkg/expvar/)

[48](#id-WakuoFLFEfk-marker)The Borg Name System (BNS) is described in [The Production Environment at Google, from the Viewpoint of an SRE](/sre-book/production-environment/).

[49](#id-JJ7umIvcZfL-marker)Recall in [Monitoring Distributed Systems](/sre-book/monitoring-distributed-systems/) the distinction between alerting on symptoms and on causes.

[50](#id-JJ7umIpTASL-marker)This 12-hour horizon is a magic number that aims to have enough information for debugging an incident in RAM for fast queries without costing *too much* RAM.

[51](#id-gA2unS4IBieSg-marker)The `service` and `zone` labels are elided here for space, but are present in the returned expression.

[52](#id-JJ7uzFpT1FL-marker)Computing the sum of rates instead of the rate of sums defends the result against counter resets or missing data, perhaps due to a task restart or failed collection of data.

[53](#id-9vburIGTzFB-marker)Despite being untyped, the majority of varz are simple counters. Borgmon’s rate function handles all the corner cases of counter resets.

[54](#id-Pj0uxF0SlFb-marker)The `service` and `zone` labels are elided for space.

[55](#id-YAJuDSrs9FV-marker)The `service` and `zone` labels are elided for space.

[Previous

Part III - Practices](/sre-book/part-III-practices/)
